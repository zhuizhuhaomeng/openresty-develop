 ...
 * register events of fd to epfd
 */
static void epoll_ctl_readd(int epfd, int fd, uint32_t events)
{
    struct epoll_event ev;
    ev.events = events;
    ev.data.fd = fd;
...
void server_run()
{
    ...
    for (;;)
    {
        ...
        if (nfds == 0) {
            if (conn_sock > 0) {
                printf("timeout: readd the client socket to epoll\n");
                epoll_ctl_readd(epfd, conn_sock,
                                EPOLLIN | EPOLLET | EPOLLRDHUP |
                                EPOLLHUP | EPOLLONESHOT);
            }
