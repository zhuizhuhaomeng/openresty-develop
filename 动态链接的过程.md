...
我们再回顾一下上面的 plt 函数。可以看到下一条指令把 常数 0压栈，然后跳转到 0x401020 去执行。我们看看 0x401020 这个地方的指令。
...
可以看到 0x403ff8 地址存储的是 _dl_runtime_resolve_xsave 这个解析 puts 的函数。关于该函数的原理可以参考 https://ypl.coffee/dl-resolve/ 。
...
按照 [这篇文章](https://ypl.coffee/dl-resolve/) 的说法，我们应该是通过 link_map->l_info 来解析 puts这个符号的。我也确实获取成功过。
但是我在这里这篇文章的时候发现 link_map->l_info 数组一直都是 0 。
...
(gdb) print *(struct link_map*)0x403ff0
$2 = {
  l_addr = 140737354129904,
  l_name = 0x7ffff7dd0b20 <_dl_runtime_resolve_xsave> "\363\017\036\372SH\211\343H\203\344\300H+%\355\272\"",
  l_ld = 0x401036 <puts@plt+6>,
...
  l_info = {0x0 <repeats 77 times>},
  l_phdr = 0x0,
  l_entry = 0,
....
  l_relro_size = 0,
